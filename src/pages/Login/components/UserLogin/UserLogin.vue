        ...
        methods: {
            submitBtn() {
                ...
                this.$refs['form'].validate((valid) => {
                    if (valid) {
                        Login(self.user.username, self.user.password).then(response => {
                            setToken(response.access_token)
                            store.getters.User.token = response.access_token
                            var Permission = [];
                            response.Permission.forEach(function (v, i) {
                                Permission.push(v.authority)
                            })
                            store.getters.User.Permission = Permission
                            self.Jump();
                        }).catch(error => {
                            console.log(error)
                        })
